...
class customAttrTest(L3NexthopTestHelper):
    '''
        Test verifies nexthop custom attribute.
    '''
    def setUp(self):
        super(customAttrTest, self).setUp()

    def runTest(self):
        print("customAttrTest")
        try:
            print("Creating nexthop with custom attribute")
            attr_value = sai_thrift_attribute_value_t(oid=self.port0_rif)
            ...
            attr_value = sai_thrift_attribute_value_t(oid=self.port1_rif)
            attr = sai_thrift_attribute_t(id=(SAI_NEXT_HOP_ATTR_END + 0),
                                          value=attr_value)
    ...
    def tearDown(self):
        super(customAttrTest, self).tearDown()
