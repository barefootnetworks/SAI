...
extern "C" {
int start_p4_sai_thrift_rpc_server_v2(int port) {
  std::cerr << "Starting SAI RPC server v2  on port " << port << std::endl;

  cookie = NULL;
  int status = pthread_create(
      &sai_thrift_rpc_thread, NULL, sai_thrift_rpc_server_thread, &port);
...
int stop_p4_sai_thrift_rpc_server_v2(void) {
